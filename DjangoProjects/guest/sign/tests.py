...
class ModelsTest(TestCase):
    def setUp(self):
        Event.objects.create(id=5, name="oneplus 3 event", status=True, limit=2000,address='shenzhen', start_time='2017-10-15 02:18:22')
        Guest.objects.create(id=6,event_id=5,realname='alen',phone='13711001101',email='alen@mail.com', sign=False)
...
class IndexPageTest(TestCase):
    ...
    def test_index_page_renders_index_template(self):
        ''' 测试 index 视图 '''
        response = self.client.get('/login_first_page/')
...
class LoginActionTest(TestCase):
    ...
    def test_login_action_username_password_null(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn(b"username or passowrd is error,please check them", response.content)
...
class EventManageTest(TestCase):
    ''' 发布会管理 '''
    def setUp(self):
        Event.objects.create(id=6,name='xiaomi5',status=True,limit=2000,address='beijing',start_time=datetime(2016,8,10,14,0,0))
        self.c = Client()
...
class GutestManageTest(TestCase):
    """嘉宾管理"""
    def setUp(self):
        Event.objects.create(id=7,name="xiaomi5",limit=2000,address='beijing',status=1,start_time=datetime(2016,8,10,14,0,0))
        Guest.objects.create(realname="alen",phone=18611001100,email='alen@mail.com',sign=0,event_id=7)
        self.c = Client()
